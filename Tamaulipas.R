...
ConcTamaulipas$Nhog <- 1

#DECILES 

#Attaching the data frame
attach(ConcTamaulipas) #this is for not writing the name of the data frame and the $ ever time

#Sort Conc according to INGCOR, FOLIOVIV, FOLIOHOG
ConcTamaulipas<- orderBy (~+ing_cor+folioviv+foliohog, data=ConcTamaulipas) #this give us the households sorted by total income

#Adding the values of the expansion FACTOR. The sum is 34 million, which is the number of households in the country.
tot_hogares<-sum(ConcTamaulipas$factor,to.data.frame=TRUE)
...
ConcTamaulipas$tam_dec<-tam_dec

# Creating Deciles of Income 

ConcTamaulipas$MAXT<-ConcTamaulipas$ing_cor #vamos a crear en esta base la variable MAXT que es una copia de la columna de ingresos

ConcTamaulipas<-ConcTamaulipas[with(ConcTamaulipas, order(rank(MAXT))),]  #lo que hicimos aqu? fue reordenar la base con respecto a MAXT. Cosa que en realidad, ya estaba.

ConcTamaulipas$ACUMULA<-cumsum(ConcTamaulipas$factor) #aqu? creamos una variable de suma acumulada del FACTOR de viviendas.
...
for(i in 1:9)
{
  a1<-ConcTamaulipas[dim(ConcTamaulipas[ConcTamaulipas$ACUMULA<tam_dec*i,])[1]+1,]$factor
  ConcTamaulipas<-rbind(ConcTamaulipas[1:(dim(ConcTamaulipas[ConcTamaulipas$ACUMULA<tam_dec*i,])[1]+1),],
                     ConcTamaulipas[(dim(ConcTamaulipas[ConcTamaulipas$ACUMULA<tam_dec*i,])[1]+1):dim(ConcTamaulipas[1])[1],])
  
  b1<-tam_dec*i-ConcTamaulipas[dim(ConcTamaulipas[ConcTamaulipas$ACUMULA<tam_dec*i,])[1],]$ACUMULA
  ConcTamaulipas[(dim(ConcTamaulipas[ConcTamaulipas$ACUMULA<tam_dec*i,])[1]+1),]$factor<-b1
  ConcTamaulipas[(dim(ConcTamaulipas[ConcTamaulipas$ACUMULA<tam_dec*i,])[1]+2),]$factor<-(a1-b1)
}

#aqu? estamos creando otra variable de suma acumulada del n?mero de hogares
ConcTamaulipas$ACUMULA2<-cumsum(ConcTamaulipas$factor)

#aqu? estamos creando una variable que se llama decil que solo tiene ceros
ConcTamaulipas$DECIL<-0
...
ConcTamaulipas[(ConcTamaulipas$ACUMULA2<=tam_dec),]$DECIL<-1
...
for(i in 1:9)
{
  ConcTamaulipas[((ConcTamaulipas$ACUMULA2>tam_dec*i)&(ConcTamaulipas$ACUMULA2<=tam_dec*(i+1))),]$DECIL<-(i+1)
}

# a lo que le qued? cero (que es la ?ltima observaci?n), ponle el decil 10
ConcTamaulipas[ConcTamaulipas$DECIL%in%"0",]$DECIL<-10

setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Tamaulipas")
write.dbf(ConcTamaulipas,file="ConcTamaulipas2010.dbf")
...
setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Tamaulipas")
write.dbf(ConcTamaulipas,file="ConcTamaulipas2018.dbf")
...
#reading the data
setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Tamaulipas")
Conc2010<-read.dbf("ConcTamaulipas2010.dbf",as.is = T)
...
write.dbf(c_DECIL_ES,file = "Tamaulipas por fuente por DECIL estimaciones 2010.dbf")
write.dbf(c_DECIL_SE,file = "Tamaulipas por fuente por DECIL errores standard 2010.dbf")
write.dbf(c_DECIL_CV,file = "Tamaulipas por fuente por DECIL CV 2010.dbf")
write.dbf(c_DECIL_LI,file = "Tamaulipas por fuente por DECIL LI 2010.dbf")
write.dbf(c_DECIL_ES,file = "Tamaulipas por fuente por DECIL LS 2010.dbf")
...
write.dbf(c_DECIL_ES,file = "Tamaulipas INDIGENA por fuente por DECIL estimaciones 2010.dbf")
write.dbf(c_DECIL_SE,file = "Tamaulipas INDIGENA por fuente por DECIL errores standard 2010.dbf")
write.dbf(c_DECIL_CV,file = "Tamaulipas INDIGENA por fuente por DECIL CV 2010.dbf")
write.dbf(c_DECIL_LI,file = "Tamaulipas INDIGENA por fuente por DECIL LI 2010.dbf")
write.dbf(c_DECIL_ES,file = "Tamaulipas INDIGENA por fuente por DECIL LS 2010.dbf")
...
write.dbf(c_DECIL_ES,file = "Tamaulipas NO por fuente por DECIL estimaciones 2010.dbf")
write.dbf(c_DECIL_SE,file = "Tamaulipas NO por fuente por DECIL errores standard 2010.dbf")
write.dbf(c_DECIL_CV,file = "Tamaulipas NO por fuente por DECIL CV 2010.dbf")
write.dbf(c_DECIL_LI,file = "Tamaulipas NO por fuente por DECIL LI 2010.dbf")
write.dbf(c_DECIL_ES,file = "Tamaulipas NO por fuente por DECIL LS 2010.dbf")
...
#reading the data
setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Tamaulipas")
Conc2018<-read.dbf("ConcTamaulipas2018.dbf",as.is = T)
...
write.dbf(c_DECIL_ES,file = "Tamaulipas Ingresos por fuente por DECIL estimaciones 2018.dbf")
write.dbf(c_DECIL_SE,file = "Tamaulipas Ingresos por fuente por DECIL errores standard 2018.dbf")
write.dbf(c_DECIL_CV,file = "Tamaulipas Ingresos por fuente por DECIL CV 2018.dbf")
write.dbf(c_DECIL_LI,file = "Tamaulipas Ingresos por fuente por DECIL LI 2018.dbf")
write.dbf(c_DECIL_ES,file = "Tamaulipas Ingresos por fuente por DECIL LS 2018.dbf")
...
write.dbf(c_DECIL_ES,file = "Tamaulipas INDIGENA Ingresos por fuente por DECIL estimaciones 2018.dbf")
write.dbf(c_DECIL_SE,file = "Tamaulipas INDIGENA Ingresos por fuente por DECIL errores standard 2018.dbf")
write.dbf(c_DECIL_CV,file = "Tamaulipas INDIGENA Ingresos por fuente por DECIL CV 2018.dbf")
write.dbf(c_DECIL_LI,file = "Tamaulipas INDIGENA Ingresos por fuente por DECIL LI 2018.dbf")
write.dbf(c_DECIL_ES,file = "Tamaulipas INDIGENA Ingresos por fuente por DECIL LS 2018.dbf")
...
write.dbf(c_DECIL_ES,file = "Tamaulipas NO Ingresos por fuente por DECIL estimaciones 2018.dbf")
write.dbf(c_DECIL_SE,file = "Tamaulipas NO Ingresos por fuente por DECIL errores standard 2018.dbf")
write.dbf(c_DECIL_CV,file = "Tamaulipas NO Ingresos por fuente por DECIL CV 2018.dbf")
write.dbf(c_DECIL_LI,file = "Tamaulipas NO Ingresos por fuente por DECIL LI 2018.dbf")
write.dbf(c_DECIL_ES,file = "Tamaulipas NO Ingresos por fuente por DECIL LS 2018.dbf")
...
Deciles2010<-read.dbf("Tamaulipas por fuente por DECIL estimaciones 2010.dbf")
names(Deciles2010)[1]<-c("ingcor2010")

Deciles2018<-read.dbf("Tamaulipas Ingresos por fuente por DECIL estimaciones 2018.dbf")
...
deciles2010No<-read.dbf("Tamaulipas NO por fuente por DECIL estimaciones 2010.dbf")
names(deciles2010No)[1]<-c("No2010")

deciles2018No<-read.dbf("Tamaulipas NO Ingresos por fuente por DECIL estimaciones 2018.dbf")
...
deciles2010indigena<-read.dbf("Tamaulipas INDIGENA por fuente por DECIL estimaciones 2010.dbf")
names(deciles2010indigena)[1]<-c("Indigena2010")

deciles2018indigena<-read.dbf("Tamaulipas INDIGENA Ingresos por fuente por DECIL estimaciones 2018.dbf")
